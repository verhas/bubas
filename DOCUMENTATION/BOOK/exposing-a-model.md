...
the same answer in every printing. A real one would send the line to a service and return what came
back. The shape is what matters, and the shape is the subject of this chapter.
...
manager reads, disagrees with, and changes. It is version-controlled. When an auditor asks why this
claim was flagged and that one was not, the answer is a threshold somebody chose and can be asked
about.

**With a verdict, the policy moves into the model.** It is not visible, not reviewable, not
explicable, and not version-controlled in any sense that helps. The auditor's question has no answer
beyond "the model said so." And the BUBAS program that existed so a subject-matter expert could own
the rule has been reduced to glue between a service and a database.
...
This is not an argument against models. It is an argument about where the seam goes.
...
where the gap is widest. An operation described as returning an opinion about a line of spending
